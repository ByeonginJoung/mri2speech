...
def get_args():
    ...
    # for the neural vocoder that we will use, WaveGlow, are using 80 channels for mel-spectrogram.
    args_edict.model.in_feat = 80 # for mgclsp, generally 25, for raw, 534, for melspectogram, 80
    
    args_edict.mseloss_weight = 1.
    
    args_edict.use_mrsloss = False
    args_edict.mrsloss_res = [128, 256]
    args_edict.mrsloss_hop = [32, 64]
    args_edict.mrsloss_win = [128, 256]
    args_edict.mrsloss_weight = 0.1

    args_edict.use_temporal_consistency = False
    args_edict.temporal_cons_weight = 0.05

    args_edict.data = edict()
    args_edict.data.feature_mode = 'melspectrogram'
    ...
    args_edict.data.augmentation.add_gaussian_std = 0.1
