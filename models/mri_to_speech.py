...
class MRI2Speech(nn.Module):
    def __init__(self,
        ...
        # during predicting audio, the model should set some of hyperparameter related to the length of audio
        # (single input frames) * (fps) / (sr) = 44100 / ( 23.18 )
        # per a image, predict 1903 audios -- if it is raw
        ...
        # if you generate the melspectrogram parameters
        #self.dense1 = nn.Linear(n_feats, int(args.data.samplingFrequency / fps / args.data.fps_control_ratio))
        if args.data.feature_mode == 'raw':
            self.dense1 = nn.Linear(n_feats, args.data.frameShift)
        elif args.data.feature_mode == 'melspectrogram':
            self.dense1 = nn.Linear(n_feats, int(args.model.in_feat * args.data.fps_control_ratio))
        else:
